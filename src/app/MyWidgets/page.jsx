...
export default function MyWidgets() {
  ...
  // Load token once
  useEffect(() => {
    const { token: fetchedToken } = getAuthToken();
    setToken(fetchedToken);
  }, []);

  // Fetch widgets
  useEffect(() => {
    const { token, username } = getAuthToken();
  ...
  // Delete widget (triggered by state)
  useEffect(() => {
    if (!deleteId || !token) return;

    const deleteWidget = async () => {
      const confirmed = confirm('Are you sure?');
      if (!confirmed) return setDeleteId(null);
    ...
    <ProtectedRoute>
      <div className="flex min-h-screen bg-gray-100">
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="max-w-7xl mx-auto px-35 py-20">
